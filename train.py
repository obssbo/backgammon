...
import agent_dqn as agent                            # student agent (this repo's agent.py)
...
def play_one_game(agent1, agent2, training=False, commentary=False, HighGameNum=False):
    ...
    if hasattr(agent1, "end_episode"): agent1.end_episode(+1 if winner == 1 else -1, final_board, perspective=+1, HighGameNum=HighGameNum)
    if hasattr(agent2, "end_episode"): agent2.end_episode(+1 if winner == -1 else -1, final_board, perspective=-1, HighGameNum=HighGameNum)

    return winner, final_board

def evaluate(agent_mod, evaluation_agent, n_eval, label="", HighGameNum=False):
    wins = 0
    # alternate who starts to reduce bias
    for i in range(n_eval):
        if i % 2 == 0:
            w, _ = play_one_game(agent_mod, evaluation_agent, training=False, commentary=False, HighGameNum=HighGameNum)
        else:
            w, _ = play_one_game(evaluation_agent, agent_mod, training=False, commentary=False, HighGameNum=HighGameNum)
            w = -w
        wins += int(w == 1)
...
def train(n_games=200_000, n_epochs=5_000, n_eval=500, eval_vs="pubeval"):
    ...
    for g in range(1, n_games + 1):
        if g > 3000:
            HighGameNum = True
        winner, final_board = play_one_game(agent, agent, training=True, commentary=False, HighGameNum=HighGameNum)
        ...
        if (g % 100) == 0:
            ...
            wr = evaluate(agent, baseline, n_eval, label=f"after {g} games", HighGameNum=HighGameNum)
            winrates.append(wr)
